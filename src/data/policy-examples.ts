...
规则 评估贷款 给定 申请人，产出：
  如果 申请人.年龄 小于 18
    返回 决定 包含 批准 将 设为 假, 理由 将 设为 「申请人未成年」, 利率 将 设为 0。
  如果 申请人.信用评分 小于 600
    返回 决定 包含 批准 将 设为 假, 理由 将 设为 「信用评分过低」, 利率 将 设为 0。
  如果 申请人.信用评分 大于 750
    返回 决定 包含 批准 将 设为 真, 理由 将 设为 「信用优秀」, 利率 将 设为 350。
  如果 申请人.信用评分 大于 700
    返回 决定 包含 批准 将 设为 真, 理由 将 设为 「信用良好」, 利率 将 设为 450。
  返回 决定 包含 批准 将 设为 真, 理由 将 设为 「标准审批」, 利率 将 设为 550。
`;
...
规则 检查资格 给定 患者，服务，产出：
  如果 非 患者.有保险
    返回 审核结果 包含 合格 将 设为 假, 覆盖率 将 设为 0, 患者费用 将 设为 服务.价格, 理由 将 设为 「无保险」。
  如果 患者.年龄 小于 18
    返回 审核结果 包含 合格 将 设为 真, 覆盖率 将 设为 90, 患者费用 将 设为 服务.价格 乘 10 除以 100, 理由 将 设为 「未成年人覆盖」。
  如果 患者.年龄 大于 65
    返回 审核结果 包含 合格 将 设为 真, 覆盖率 将 设为 85, 患者费用 将 设为 服务.价格 乘 15 除以 100, 理由 将 设为 「老年人覆盖」。
  返回 审核结果 包含 合格 将 设为 真, 覆盖率 将 设为 70, 患者费用 将 设为 服务.价格 乘 30 除以 100, 理由 将 设为 「标准覆盖」。
`;
...
规则 生成报价 给定 驾驶员，车辆，产出：
  如果 驾驶员.年龄 小于 18
    返回 报价 包含 批准 将 设为 假, 保费 将 设为 0, 免赔额 将 设为 0, 理由 将 设为 「驾驶员未满18岁」。
  如果 驾驶员.事故数 大于 3
    返回 报价 包含 批准 将 设为 假, 保费 将 设为 0, 免赔额 将 设为 0, 理由 将 设为 「事故过多」。
  令 基础保费 为 计算基础(驾驶员, 车辆)。
  令 风险系数 为 计算风险(驾驶员)。
  令 最终保费 为 基础保费 乘 风险系数 除以 100。
  返回 报价 包含 批准 将 设为 真, 保费 将 设为 最终保费, 免赔额 将 设为 500, 理由 将 设为 「已批准」。
...
规则 检测欺诈 给定 交易，历史，产出：
  如果 交易.金额 大于 1000000
    返回 欺诈结果 包含 可疑 将 设为 真, 风险评分 将 设为 100, 理由 将 设为 「超大额交易」。
  如果 历史.可疑次数 大于 5
    返回 欺诈结果 包含 可疑 将 设为 真, 风险评分 将 设为 85, 理由 将 设为 「高度可疑活动」。
  如果 历史.账龄 小于 30
    返回 欺诈结果 包含 可疑 将 设为 真, 风险评分 将 设为 70, 理由 将 设为 「新账户风险」。
  如果 交易.金额 大于 历史.平均金额 乘 10
    返回 欺诈结果 包含 可疑 将 设为 真, 风险评分 将 设为 60, 理由 将 设为 「异常金额」。
  返回 欺诈结果 包含 可疑 将 设为 假, 风险评分 将 设为 10, 理由 将 设为 「正常交易」。
`;
...
规则 评估申请 给定 申请人，申请，产出：
  如果 申请人.年龄 小于 21
    返回 决定 包含 批准 将 设为 假, 批准额度 将 设为 0, 利率 将 设为 0, 理由 将 设为 「年龄未满21岁」。
  如果 申请人.信用评分 小于 550
    返回 决定 包含 批准 将 设为 假, 批准额度 将 设为 0, 利率 将 设为 0, 理由 将 设为 「信用评分过低」。
  如果 申请人.现有卡数 大于 5
    返回 决定 包含 批准 将 设为 假, 批准额度 将 设为 0, 利率 将 设为 0, 理由 将 设为 「现有卡数过多」。
  令 额度 为 确定额度(申请人, 申请)。
  令 利率值 为 确定利率(申请人)。
  返回 决定 包含 批准 将 设为 真, 批准额度 将 设为 额度, 利率 将 设为 利率值, 理由 将 设为 「已批准」。
