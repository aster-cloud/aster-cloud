...
【函数】检查资格 包含 患者，服务，产出：
  如果 非 患者.有保险：
    返回 结果 包含 合格 = 假, 覆盖率 = 0, 患者费用 = 服务.价格, 理由 = 「无保险」。
  如果 患者.年龄 小于 18：
    返回 结果 包含 合格 = 真, 覆盖率 = 90, 患者费用 = 服务.价格 乘 10 除以 100, 理由 = 「未成年人覆盖」。
  如果 患者.年龄 大于 65：
    返回 结果 包含 合格 = 真, 覆盖率 = 85, 患者费用 = 服务.价格 乘 15 除以 100, 理由 = 「老年人覆盖」。
  返回 结果 包含 合格 = 真, 覆盖率 = 70, 患者费用 = 服务.价格 乘 30 除以 100, 理由 = 「标准覆盖」。
`;
