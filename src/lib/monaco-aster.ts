...
export function registerAsterLanguage(monaco: Monaco): void {
  ...
  monaco.languages.setMonarchTokensProvider('aster-cnl', {
    ...
    // English keywords（与 aster-lang-lexicons.ts EN_US_LEXICON 保持同步）
    keywords: [
      'Module',
      'use',
      'as',
    ...
    // Chinese keywords（与 aster-lang-lexicons.ts ZH_CN_LEXICON 保持同步）
    chineseKeywords: [
      '模块',
      '引用',
      '作为',
    ...
    // German keywords（与 aster-lang-lexicons.ts DE_DE_LEXICON 保持同步）
    germanKeywords: [
      'Modul',
      'verwende',
      'als',
