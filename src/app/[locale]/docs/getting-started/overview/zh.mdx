---
title: "概述"
description: "Aster 策略引擎是一个生产级 REST 服务，用于评估以 Aster CNL（受控自然语言）编写的业务规则。它基于 Quarkus 构建，专为低延迟、多租户部署而设计。"
updated: 2026-06-02
apiVersion: v1
---
...
修改状态或携带敏感输入的请求还应包含 HMAC 签名请求头。完整签名协议请参阅[认证](/zh/docs/getting-started/authentication)。
...
策略可以作为源文本提交（即时评估），也可以作为预编译和缓存的已存储策略对象。详情请参阅 [API 参考](/zh/docs/api/policies/evaluate)。

## 下一步

- [认证](/zh/docs/getting-started/authentication) — 配置租户 ID、HMAC 签名和 RBAC 角色
- [快速开始](/zh/docs/getting-started/quickstart) — 五分钟内运行你的第一次策略评估
- [错误处理](/zh/docs/getting-started/errors) — 了解错误响应格式和常见故障模式
