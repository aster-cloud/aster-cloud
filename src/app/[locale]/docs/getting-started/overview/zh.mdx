...
修改状态或携带敏感输入的请求还应包含 HMAC 签名请求头。完整签名协议请参阅[认证](/docs/getting-started/authentication)。
...
策略可以作为源文本提交（即时评估），也可以作为预编译和缓存的已存储策略对象。详情请参阅 [API 参考](/docs/api/policies/evaluate)。

## 下一步

- [认证](/docs/getting-started/authentication) — 配置租户 ID、HMAC 签名和 RBAC 角色
- [快速开始](/docs/getting-started/quickstart) — 五分钟内运行你的第一次策略评估
- [错误处理](/docs/getting-started/errors) — 了解错误响应格式和常见故障模式
