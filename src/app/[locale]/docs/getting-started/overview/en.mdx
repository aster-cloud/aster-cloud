---
title: "Overview"
description: "What Aster is and how its policy API fits together — author rules in controlled natural language, evaluate them over REST, and audit every decision."
updated: 2026-06-02
apiVersion: v1
---
...
Requests that modify state or carry sensitive inputs should also include HMAC signing headers. See [Authentication](/docs/getting-started/authentication) for the full signing protocol.
| Header               | Description                                    |
|----------------------|------------------------------------------------|
...
Policies are submitted either as source text (evaluated on-the-fly) or as stored policy objects that are pre-compiled and cached. See the [API Reference](/docs/api/policies/evaluate) for details on both modes.
## Next Steps

- [Authentication](/docs/getting-started/authentication) — configure tenant IDs, HMAC signing, and RBAC roles
- [Quick Start](/docs/getting-started/quickstart) — run your first policy evaluation in under five minutes
- [Error Handling](/docs/getting-started/errors) — understand error response formats and common failure modes
