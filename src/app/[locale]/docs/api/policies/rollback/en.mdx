---
title: "Rollback Policy to Version"
description: "Revert a policy to a previously saved version. The engine atomically swaps the active version so that subsequent evaluation requests immediately use the…"
updated: 2026-06-02
apiVersion: v1
---
...
|-------|------|----------|-------------|
| `targetVersion` | `number` | Yes | The version number to restore. Must be a positive integer that exists in the policy's version history. Use the [versions endpoint](/docs/api/policies/versions) to enumerate available versions. |
