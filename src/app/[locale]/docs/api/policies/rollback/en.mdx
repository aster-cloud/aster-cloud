...
|-------|------|----------|-------------|
| `targetVersion` | `number` | Yes | The version number to restore. Must be a positive integer that exists in the policy's version history. Use the [versions endpoint](/docs/api/policies/versions.md) to enumerate available versions. |
