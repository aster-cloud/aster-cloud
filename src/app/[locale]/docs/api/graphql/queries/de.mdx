...
All queries are read-only operations sent as `POST /graphql`. Queries require HMAC request signing and an `X-Tenant-Id` header. See [Authentication](/de/docs/getting-started/authentication) for details on computing the signature, and [Overview](/de/docs/api/graphql/overview) for request format details.
