...
All queries are read-only operations sent as `POST /graphql`. Queries require HMAC request signing and an `X-Tenant-Id` header. See [Authentication](/zh/docs/getting-started/authentication) for details on computing the signature, and [Overview](/zh/docs/api/graphql/overview) for request format details.
