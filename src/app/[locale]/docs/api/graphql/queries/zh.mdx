...
所有查询都是以 `POST /graphql` 发送的只读操作。查询需要 HMAC 请求签名和 `X-Tenant-Id` 请求头。签名计算细节参阅 [认证](/docs/getting-started/authentication)；请求格式参阅 [概览](/docs/api/graphql/overview)。
...
```bash [curl — evaluatePersonalLoan 示例]
curl -X POST https://policy.aster-lang.dev/graphql \
  -H "Content-Type: application/json" \
  -H "X-Tenant-Id: acme-corp" \
...
```js [JavaScript — evaluatePersonalLoan 示例]
const response = await fetch('https://policy.aster-lang.dev/graphql', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
