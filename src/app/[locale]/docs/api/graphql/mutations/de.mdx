...
Mutations are write operations that create, update, or delete resources. All mutations must be sent as `POST /graphql`. Write operations accept an optional `Idempotency-Key` header to prevent duplicate execution on retry. See [Overview](/de/docs/api/graphql/overview) for full request format details.
...
Mutation errors follow the same GraphQL error format described in [Overview](/de/docs/api/graphql/overview#error-handling). Common mutation-specific error codes:
