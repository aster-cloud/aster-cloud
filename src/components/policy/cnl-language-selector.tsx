  ...
  return (
    <div ref={containerRef} className={`relative inline-block ${className}`}>
      ...
      <button
        ...
        className={`inline-flex items-center justify-between gap-2 rounded-lg border border-border-strong bg-bg shadow-sm transition-all duration-200 hover:bg-bg-subtle hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary ${
      ...
      >
        <span className="inline-flex items-center gap-1.5">
          ...
          {!compact && <span className="text-fg">{selectedOption.label}</span>}
          {compact && <span className="text-fg font-medium">{selectedOption.shortLabel}</span>}
        </span>
        <svg
      ...
      {isOpen && (
        <div
          className="absolute z-20 mt-1 w-40 origin-top-left rounded-lg bg-bg shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
          role="listbox"
        >
          <div className="py-1">
