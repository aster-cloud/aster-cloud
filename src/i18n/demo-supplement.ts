...
export const DEMO_SUPPLEMENT: Record<Locale, MessageTree> = {
  ...
  zh: {
    ...
    poemDemoPage: {
      ...
      run: {
        title: '它真的算——运行输出诗名',
        hint: '点任意一次「吟」，运行入口规则「地上霜」。别名与字面量宏在 canonicalize 表层展开，引擎真求值，输出诗名「静夜思」。',
        button: '吟 · 第 {n} 遍 ▶',
      },
      result: {
        title: '第 {n} 遍吟出的',
        note: '这运行在你的浏览器里、用驱动信贷与扑克 demo 的同一套 TypeScript 引擎——「思故乡」经字面量宏在表层展开成字符串「静夜思」，规则真编译真执行，不是预先写死的字符串。',
      },
      cta: {
