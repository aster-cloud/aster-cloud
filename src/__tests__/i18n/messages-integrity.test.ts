...
function loadAndCount(locale: string): ParsedFile {
...
describe('messages JSON 完整性', () => {
  for (const locale of LOCALES) {
    describe(`messages/${locale}.json`, () => {
      it('JSON parse 成功', () => {
        expect(() => loadAndCount(locale)).not.toThrow();
      });

      it('top-level 键无重复（防止 v6/v7/v8 dashboard 重复 bug 复发）', () => {
        const { topLevelKeyCounts } = loadAndCount(locale);
        const dups: string[] = [];
        ...
        expect(dups, `${locale}.json 有重复 top-level 键：${dups.join(', ')}`).toEqual([]);
      });
      ...
      it('dashboard.welcomeBack 存在（修复 Bug-2 的核心键）', () => {
        const { json } = loadAndCount(locale);
  ...
  it('三语 top-level 键集合一致', () => {
    const en = new Set(Object.keys(loadAndCount('en').json));
    const zh = new Set(Object.keys(loadAndCount('zh').json));
    const de = new Set(Object.keys(loadAndCount('de').json));
